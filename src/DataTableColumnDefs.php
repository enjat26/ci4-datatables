...
class DataTableColumnDefs
{
    ...
    public function initFromBuilder($baseBuilder)
    {
        ...
        $QBSelect = Helper::getObjectPropertyValue($builder, 'query');
        $QBSelect = Helper::getObjectPropertyValue($QBSelect, 'columns');

        if (!empty($QBSelect)) {
            // $baseSQL    = $builder->getCompiledSelect(false);
            $baseSQL    = $builder->toSql();
            // die(print_r($baseSQL));
            $parser     = new \PHPSQLParser\PHPSQLParser();
            $sqlParsed  = $parser->parse($baseSQL);
            // die(print_r($sqlParsed));
            ...
            $QBFrom = Helper::getObjectPropertyValue($builder, 'query');
            die(print_r($QBFrom));
            $QBFrom = Helper::getObjectPropertyValue($QBFrom, 'from');
            // $QBJoin   = Helper::getObjectPropertyValue($builder, 'QBJoin');
            $QBJoin   = Helper::getObjectPropertyValue($builder, 'query');
            $QBJoin   = Helper::getObjectPropertyValue($QBJoin, 'joins');
